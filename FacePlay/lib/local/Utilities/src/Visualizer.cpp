	...
	const std::vector<std::pair<std::string, double> >& au_occurences)
{
	if (au_intensities.size() > 0 || au_occurences.size() > 0)
	{
		...
		// unsigned szz = au_names.size();
		std::vector<std::string> au_names_vec;

		for (auto au_name : au_names) {
			std::string ss = au_name.substr(2, 2);
			int au_name_to_numb = std::stoi(ss);
			au_names_to_numbers.push_back(au_name_to_numb);
			au_names_vec.push_back(au_name);
		}
/*
		std::vector<int> indices;
		int xx = 0;
		for (unsigned i = 0; i < au_names_vec.size()/2; i++) {
			indices.push_back(xx);
			xx++;
		}

		sort(indices.begin(), indices.end(), [&](int i, int j) {return au_names_to_numbers[i] < au_names_to_numbers[j]; });
		int sz = au_names_vec.size() / 2;
		for (unsigned i = 0; i < au_names_vec.size()/2; i++) {
			au_names_vec[i] = au_names_vec[sz + indices[i]];
		}

		au_names_vec.erase(au_names_vec.begin() + sz + 1, au_names_vec.end() + 1);*/
		
		// au_names_vec.shrink_to_fit();
		// std::cout << au_names_vec.size() << "\t" << au_names_vec.capacity() << "\n";
		...
		// au_frame_data.name1 = au_names_to_numbers[0];
		au_frame_data.reg01 = aus[au_names_vec[0]].second;
		au_frame_data.present01 = aus[au_names_vec[0]].first;

		// au_frame_data.name2 = au_names_to_numbers[1];
		au_frame_data.reg02 = aus[au_names_vec[1]].second;
		au_frame_data.present02 = aus[au_names_vec[1]].first;

		// au_frame_data.name3 = au_names_to_numbers[2];
		au_frame_data.reg04 = aus[au_names_vec[2]].second;
		au_frame_data.present04 = aus[au_names_vec[2]].first;

		// au_frame_data.name4 = au_names_to_numbers[3];
		au_frame_data.reg05 = aus[au_names_vec[3]].second;
		au_frame_data.present05 = aus[au_names_vec[3]].first;

		// au_frame_data.name5 = au_names_to_numbers[4];
		au_frame_data.reg06 = aus[au_names_vec[4]].second;
		au_frame_data.present06 = aus[au_names_vec[4]].first;

		// au_frame_data.name6 = au_names_to_numbers[5];
		au_frame_data.reg07 = aus[au_names_vec[5]].second;
		au_frame_data.present07 = aus[au_names_vec[5]].first;

		// au_frame_data.name7 = au_names_to_numbers[6];
		au_frame_data.reg09 = aus[au_names_vec[6]].second;
		au_frame_data.present09 = aus[au_names_vec[6]].first;

		// au_frame_data.name8 = au_names_to_numbers[7];
		au_frame_data.reg10 = aus[au_names_vec[7]].second;
		au_frame_data.present10 = aus[au_names_vec[7]].first;

		// au_frame_data.name9= au_names_to_numbers[8];
		au_frame_data.reg12 = aus[au_names_vec[8]].second;
		au_frame_data.present12 = aus[au_names_vec[8]].first;

		// au_frame_data.name10= au_names_to_numbers[9];
		au_frame_data.reg14 = aus[au_names_vec[9]].second;
		au_frame_data.present14 = aus[au_names_vec[9]].first;

		// au_frame_data.name11 = au_names_to_numbers[10];
		au_frame_data.reg15 = aus[au_names_vec[10]].second;
		au_frame_data.present15 = aus[au_names_vec[10]].first;

		// au_frame_data.name12 = au_names_to_numbers[11];
		au_frame_data.reg17 = aus[au_names_vec[11]].second;
		au_frame_data.present17 = aus[au_names_vec[11]].first;

		// au_frame_data.name13 = au_names_to_numbers[12];
		au_frame_data.reg20 = aus[au_names_vec[12]].second;
		au_frame_data.present20 = aus[au_names_vec[12]].first;

		// au_frame_data.name14 = au_names_to_numbers[13];
		au_frame_data.reg23 = aus[au_names_vec[13]].second;
		au_frame_data.present23 = aus[au_names_vec[13]].first;

		// au_frame_data.name15 = au_names_to_numbers[14];
		au_frame_data.reg25 = aus[au_names_vec[14]].second;
		au_frame_data.present25 = aus[au_names_vec[14]].first;

		// au_frame_data.name16 = au_names_to_numbers[15];
		au_frame_data.reg26 = aus[au_names_vec[15]].second;
		au_frame_data.present26 = aus[au_names_vec[15]].first;

		// au_frame_data.name17 = au_names_to_numbers[16];
		au_frame_data.reg28 = aus[au_names_vec[16]].second;
		au_frame_data.present28 = aus[au_names_vec[16]].first;

		// au_frame_data.name18 = au_names_to_numbers[17];
		au_frame_data.reg45 = aus[au_names_vec[17]].second;
		au_frame_data.present45 = aus[au_names_vec[17]].first;
