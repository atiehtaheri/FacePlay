...
bool SequenceCapture::OpenWebcam(int device, int image_width, int image_height, float fx, float fy, float cx, float cy)
{
	...
	std::cout << "WIDTHHHHHH:  " << this->frame_width << "\t HEIGHTTTTT:  " << this->frame_height << "\n";
	if (!capture.isOpened())
	{
...
cv::Mat SequenceCapture::GetNextFrame()
{
	...
	else
	{
		...
		//capture.grab();
		cv::Mat resized;
		//bool success = capture.retrieve(resized, 0);
		 bool success = capture.read(latest_frame);
		// std::cout << "SUCCESSSSSSSSS:  " << success << "\n";
		//latest_frame 
		// cv::Rect ROI = cv::Rect(0, 0, 400, 300);
		// resized = resized(ROI);
		 // cv::resize(resized, latest_frame, cv::Size(640, 480));
